...
namespace Crew_Planning
{
    public class CreateNetWork
    {
        ...
        {
            //path = "Server = PC-201606172102\\SQLExpress;DataBase = 上海算例;Integrated Security = true";
        ...
        public void LoadData(DataSet Ds ,int Days, int numTrain, int numCrew)
        {
            ...
            for (i = 0; i < numCrew;i++)
            {
                ...
                crew.OutCost = Convert.ToDouble(Dt.Rows[i]["出动成本原始"]);
        ...
        {
            string f1 = System.Environment.CurrentDirectory + "\\DATA\\京津\\Timetable.csv";
            string f2 = System.Environment.CurrentDirectory + "\\DATA\\京津\\CrewBase.csv";
            string f3 = System.Environment.CurrentDirectory + "\\DATA\\京津\\Crew.csv";
        ...
        public void CreateT_S_NetWork()
        {
            ...
            foreach (Station station in StationList)
            {
                ...
                for (i = 0; i < StationSortList.Count - 1; i++)
                {
                    ...
                    else if ((StationSortList[i].TimeCode < 1440 && StationSortList[i + 1].TimeCode < 1440) ||
                             (StationSortList[i].TimeCode >= 1440 && StationSortList[i + 1].TimeCode >= 1440))
                    {
                        T_S_Arc t_s_arc = new T_S_Arc();
                        t_s_arc.ID = k;
                        t_s_arc.LineID = 0;
                    ...
                    //for (j = i + 1; j < StationSortList.Count; j++)
                    //{
                    //    if (StationSortList[i].TimeCode <= 1440 && StationSortList[j].TimeCode > 1440 && station.ReMainCrew == 1
                    //        && StationSortList[i].PointType == 2 && StationSortList[j].PointType == 1
                    //        )
                    //    {
                    //        T_S_Arc t_s_arc = new T_S_Arc();
                    //        t_s_arc.ID = k;
                    ...
                    //        t_s_arc.TrainCode = "";
                    //        t_s_arc.StartPointID = StationSortList[i].ID;
                    //        t_s_arc.EndPointID = StationSortList[j].ID;
                    //        t_s_arc.StartStaCode = StationSortList[i].StaCode;
                    //        t_s_arc.EndStaCode = StationSortList[j].StaCode;
                    //        t_s_arc.StartTimeCode = StationSortList[i].TimeCode;
                    //        t_s_arc.EndTimeCode = StationSortList[j].TimeCode;
                    //        t_s_arc.ArcType = 22;//22——夜间连接弧
                    //        T_S_ArcList.Add(t_s_arc);
                    //        k++;
                    //    }
                    //    else if (StationSortList[i].PointType == 2 && StationSortList[j].PointType == 1 &&
                    //             ((StationSortList[i].TimeCode < 1440 && StationSortList[j].TimeCode < 1440) ||
                    //              (StationSortList[i].TimeCode >= 1440 && StationSortList[j].TimeCode >= 1440)))
                    //    {
                    //        T_S_Arc t_s_arc = new T_S_Arc();
                    //        t_s_arc.ID = k;
        ...
        public List<T_S_S_Arc> CreateT_S_S_NetWork(int mindrivetime,int maxdrivetime,int maxconntime,int mintranslation,int minrelaxtime,int maxrelaxtime,int minoutrelaxtime,int maxoutrelaxtime,int mindaycrewtime,int maxdaycrewtime)
        {
            ...
            //int minmealtime = 35, maxmealtime = 100;//不考虑 windows
            //int STLunch = 480, ETLunch = 840, STDinner = 900, ETDinner = 1260; //不考虑 windows
            ...
            do
            {
                ...
                //if(t_s_s_notequeue.ID==5562)
                //{
                //    int aaa = 2;
                //}
                //t_s_s_notequeue = (T_S_S_Node)QueueList[0];
                ...
                for (i = 0; i < T_S_ArcList.Count; i++)
                {
                    ...
                    {                        
                        ...
                        if (t_s_s_notequeue.TimeCode - t_s_s_notequeue.CumuDayCrewTime < STLunch + minmealtime)
                        {
                            windowofO = 1;
                            //当前点在上午，午饭晚饭均没吃才允许创建
                            if (t_s_s_notequeue.TimeCode < STLunch 
								&& t_s_s_notequeue.CumuLunch == 0 
								&& t_s_s_notequeue.CumuDinner == 0)
                            { iscreate = true; }
                            //当前点在午饭时间段，允许创建，这样才能创建用餐弧
                            else if (t_s_s_notequeue.TimeCode >= STLunch 
								&& t_s_s_notequeue.TimeCode <= ETLunch)
                            { iscreate = true; }
                            //当前点在下午，吃过午饭允许创建
                            else if (t_s_s_notequeue.TimeCode > ETLunch 
								&& t_s_s_notequeue.TimeCode < STDinner 
								&& t_s_s_notequeue.CumuLunch == 2 
								&& t_s_s_notequeue.CumuDinner == 0)
                            { iscreate = true; }
                            //当前点在晚饭时间段，允许创建，这样才能创建用餐弧
                            else if (t_s_s_notequeue.TimeCode >= STDinner 
								&& t_s_s_notequeue.TimeCode <= ETDinner)
                            { iscreate = true; }
                            //当前点在晚间，午饭晚饭均吃过才允许创建
                            else if (t_s_s_notequeue.TimeCode > ETDinner 
								&& t_s_s_notequeue.CumuLunch == 2 
								&& t_s_s_notequeue.CumuDinner == 2)
                            { iscreate = true; }
                        }
                        //始发点在午餐段或下午,司机吃午饭后再出乘
                        else if (t_s_s_notequeue.TimeCode - t_s_s_notequeue.CumuDayCrewTime >= STLunch + minmealtime 
							&& t_s_s_notequeue.TimeCode - t_s_s_notequeue.CumuDayCrewTime <= STDinner + minmealtime)
                        {
                            windowofO = 2;
                        ...
                        if (iscreate == true)
                        {
                            if (T_S_ArcList[i].EndTimeCode - T_S_ArcList[i].StartTimeCode 
									+ t_s_s_notequeue.CumuDriveTime <= mindrivetime 
								&& T_S_ArcList[i].EndTimeCode - T_S_ArcList[i].StartTimeCode 
									+ t_s_s_notequeue.CumuDayCrewTime <= maxdaycrewtime)//这个弧被执行完时，其累计运行时间不超过单次驾驶时长
                            {
                                isdrive = 1;//必须驾驶
                            }
									...
									+ t_s_s_notequeue.CumuDriveTime > mindrivetime 
								&& T_S_ArcList[i].EndTimeCode - T_S_ArcList[i].StartTimeCode 
									+ t_s_s_notequeue.CumuDriveTime <= maxdrivetime)
                            {
                                isdrive = 2;//可以驾驶也可以休息
                            }
                            else
                            ...
                            if (T_S_ArcList[i].EndTimeCode - T_S_ArcList[i].StartTimeCode + t_s_s_notequeue.CumuConnTime
								<= maxconntime)
                            {
                                isconn = true;
                            }
                            ...
                            if (T_S_ArcList[i].EndTimeCode - T_S_ArcList[i].StartTimeCode 
								+ t_s_s_notequeue.CumuDayCrewTime-t_s_s_notequeue.CumuConnTime 
								<= maxdaycrewtime)
                            {
                                isdaycrew = true;
                            }
                            ...
                            if (T_S_ArcList[i].ArcType == 21 
								&& t_s_s_notequeue.PointType != 31 
								&& T_S_ArcList[i].EndTimeCode - T_S_ArcList[i].StartTimeCode 
									+ t_s_s_notequeue.CumuDayCrewTime <= maxdaycrewtime)//此弧是日间连接弧
                            {
                                //1、需要间休，t_s_s_notequeue.Relax：0-不需要休息；1-需要间休；2-间休可休可不休；3-需要外段驻班休；4-外段驻班可休克不休（在最小休息时间和最大休息时间之间）。2、需要接续
                                //需要间休
                                if (isdaycrew == true 
									&& (isdrive != 1 
										|| t_s_s_notequeue.Relax == 1 
										|| t_s_s_notequeue.Relax == 2)
									)//三种状态分别为：1、刚好需要休息；2、已经休息，但还未休息够；3、休息够了，但还没达到最大间休时间
                                #region
                                {
                                    //1、休息时间小于最小休息时间要求，必需休息（1、刚好需要休息；2、已经休息，但还未休息够；）
                                    if (t_s_s_notequeue.CumuConnTime + T_S_ArcList[i].EndTimeCode
										- T_S_ArcList[i].StartTimeCode <= minrelaxtime 
										&& (t_s_s_notequeue.CumuConnTime <= maxconntime 
											&& t_s_s_notequeue.PointType != 22 
											|| t_s_s_notequeue.PointType == 22)
                                    ...
                                    else if (t_s_s_notequeue.CumuConnTime + T_S_ArcList[i].EndTimeCode 
											- T_S_ArcList[i].StartTimeCode >= minrelaxtime 
										&& t_s_s_notequeue.CumuConnTime + T_S_ArcList[i].EndTimeCode 
                                    ...
                                    //超了，休息任务完成
                                    else if (t_s_s_notequeue.Relax == 2 
										&& t_s_s_notequeue.CumuConnTime + T_S_ArcList[i].EndTimeCode 
										- T_S_ArcList[i].StartTimeCode > maxrelaxtime)
                                    {
                                ...
                                #endregion
                                else if (t_s_s_notequeue.CumuDayCrewTime-t_s_s_notequeue.CumuConnTime >= mindaycrewtime 
									|| t_s_s_notequeue.Relax == 3 
									|| t_s_s_notequeue.Relax == 4)//满足交路段时长约束，可以休息，也可以接续
                                #region
                                {
                                    //休息不足
                                    if (t_s_s_notequeue.CumuConnTime + T_S_ArcList[i].EndTimeCode 
										- T_S_ArcList[i].StartTimeCode < minoutrelaxtime)
                                    {
                                        T_S_S_Node t_s_s_note = new T_S_S_Node();
                                        t_s_s_note.ID = k1;
                                        t_s_s_note.RountingID = t_s_s_notequeue.RountingID;
                                    ...
                                    //超额休息
                                    else if (t_s_s_notequeue.Relax == 4 
										&& t_s_s_notequeue.CumuConnTime + T_S_ArcList[i].EndTimeCode 
										- T_S_ArcList[i].StartTimeCode >= maxoutrelaxtime)
                                    {
                                ...
                                #region
                                {
                                    if (t_s_s_notequeue.PointType != 24 
										&& isconn == true)//若不是用餐弧
                                    {
                                        T_S_S_Node t_s_s_note = new T_S_S_Node();
                                        t_s_s_note.ID = k1;
                                    ...
                                    if (t_s_s_notequeue.CumuLunch != 2
										&& windowofO == 1
										&&(t_s_s_notequeue.PointType == 1 
											|| t_s_s_notequeue.PointType == 24) 
										&& t_s_s_notequeue.TimeCode >= STLunch 
										&& t_s_s_notequeue.TimeCode + T_S_ArcList[i].EndTimeCode 
                                    ...
                                    else if (t_s_s_notequeue.CumuDinner != 2
										&& windowofO != 3 
										&&(t_s_s_notequeue.PointType == 1 
											|| t_s_s_notequeue.PointType == 24) 
										&& t_s_s_notequeue.TimeCode >= STDinner 
                            ...
                            //夜间弧：只要不是乘务基地所在站,并且该站有能力停驻乘务组，就必须过夜
								...
								&& t_s_s_notequeue.PointType != 31 
								&& T_S_ArcList[i].EndTimeCode - T_S_ArcList[i].StartTimeCode 
								+ t_s_s_notequeue.CumuDayCrewTime <= maxdaycrewtime)
                            {
                                #region
                              
                                //乘务组未在乘务基地过夜，外段驻班:不是本站，且时长不超
                                if (t_s_s_notequeue.StaCode != t_s_s_notequeue.OStation 
									&& t_s_s_notequeue.CumuConnTime + T_S_ArcList[i].EndTimeCode 
									- T_S_ArcList[i].StartTimeCode <= maxoutrelaxtime)
                                {
                                    T_S_S_Node t_s_s_note = new T_S_S_Node();
                             ...
                             //运行弧：只要不是间修或外段驻班休不足就可以担当任务
								...
								+ t_s_s_notequeue.CumuDayCrewTime <= maxdaycrewtime
								&& t_s_s_notequeue.CumuLunch != 1 
								&& t_s_s_notequeue.CumuDinner !=1 
								&& isdaycrew == true 
								&& T_S_ArcList[i].ArcType == 1 
								&& isdrive != 3 
								&& t_s_s_notequeue.Relax != 1 
								&& t_s_s_notequeue.Relax != 3 
								&& (t_s_s_notequeue.PointType != 23 
									|| t_s_s_notequeue.PointType == 23 &
									& t_s_s_notequeue.Relax == 4)
								)
                            {
                                #region 回溯
                             ...
                             保证每一个可行解的可行性*/
                                for (l = 0; l < t_s_s_notequeue.PassLine.Count;l++ )
                                {
                                    ...
                                    if (driveornot == true)
                                    {
                                        ...
                                        #endregion
                                        //不同交路，换乘
                                        else if (t_s_s_notequeue.RountingID != T_S_ArcList[i].RountingID 
											&& t_s_s_notequeue.CumuConnTime >= mintranslation)
                                        #region
                                        {
                                            T_S_S_Node t_s_s_note = new T_S_S_Node();
                                            t_s_s_note.ID = k1;
                            ...
                            else if (T_S_ArcList[i].ArcType == 31)
                            {
                                ...
                                t_s_s_note.Relax = 0;
                                //t_s_s_note.Price = T_S_ArcList[i].EndTimeCode - T_S_ArcList[i].StartTimeCode;
                                t_s_s_note.Price = 1440;// T_S_ArcList[i].EndTimeCode;//60;
                                t_s_s_note.PervCumuConnTime = t_s_s_notequeue.CumuConnTime;
                                t_s_s_note.PervCumuDriveTime = t_s_s_notequeue.CumuDriveTime;
                                t_s_s_note.PervCumuDayCrewTime = t_s_s_notequeue.CumuDayCrewTime;
                                t_s_s_note.PervCumuLunch = t_s_s_notequeue.CumuLunch;
								...
								&& T_S_ArcList[i].EndStaCode == t_s_s_notequeue.OStation 
								&& t_s_s_notequeue.PointType == 1 
								&& t_s_s_notequeue.CumuDayCrewTime >= mindaycrewtime 
								&& t_s_s_notequeue.CumuDayCrewTime <= maxdaycrewtime)
                            {
                                T_S_S_Node t_s_s_note = new T_S_S_Node();
                                foreach (CrewBase crewbase in CrewBaseList)
                            ...
                            else if (T_S_ArcList[i].ArcType == 33)
                            {
                                ...
                                t_s_s_note.TrainCode = T_S_ArcList[i].TrainCode;
                                t_s_s_note.Relax = 1;
                                t_s_s_note.Price = 1400;//150;//价格一定要和正常的交路值差不多
                                t_s_s_note.PervCumuConnTime =t_s_s_notequeue.CumuConnTime;
                                t_s_s_note.PervCumuDriveTime =t_s_s_notequeue.CumuDriveTime;
                                t_s_s_note.PervCumuDayCrewTime =t_s_s_notequeue.CumuDayCrewTime;
                                t_s_s_note.PervCumuLunch =t_s_s_notequeue.CumuLunch;
                ...
                //    //T_S_S_Node node = new T_S_S_Node();
                //    //T_S_S_NodeTempList.Remove(t_s_s_notequeue.ID);
                //    //k++;
                //    resent = t_s_s_notequeue.ID;
                //    do
                //    {
                //        foreach (KeyValuePair<int, T_S_S_Node> node in T_S_S_NodeTempList)
                //        {
                ...
                //        {
                //            deleteindex = resent;
                //            resent = T_S_S_NodeTempList[deleteindex].PervID;
                //            T_S_S_NodeTempList.Remove(deleteindex);
                //        }
                //    }
                //    while (nnext == 0);
                //}
            ...
            //T_S_S_NoteList = T_S_S_NodeTempList;
            #endregion 
            //剪枝
             int d = 0;
            QueueList.Clear();
            ...
            foreach (CrewBase crewbase in CrewBaseList)
            {
                for (i = 0; i < T_S_S_NodeODTempList.Count; i++)
                {
                    ...
                    {
                        if (T_S_S_NodeODTempList[i].PervID == 5562 && T_S_S_NodeODTempList[i].ID==1)
                        {
                            int a = 111;
                        }
            ...
            foreach (T_S_S_Node note in DPointList)
            {
                ...
                do
                {
                    ...
                    {                       
                        ...
                        d = T_S_S_NodeTempList[d].PervID;
                    }
                    //for (i = T_S_S_NodeTempList.Count - 1; i >= 0; i--)
                    //{
                    //    if (T_S_S_NodeTempList[i].ID == d)
                    //    {
                    //        //T_S_S_NodeDelList.Add(T_S_S_NodeTempList[i]);
                    //        d = T_S_S_NodeTempList[i].PervID;
                    //        T_S_S_NodeTempList[i].Isfeasible = 1;
                    //        //if (d == -1) { break; }
            ...
            //CrewBaseList.Count+T_S_S_NodeTempList.Count中的CrewBaseList.Count位TSS网络虚拟时空状态起点的个数，剩下的均为连续标号
            ...
            {
                T_S_S_NoteList.Add(T_S_S_NodeDelList[i]);
            }
            //删除重复状态
            #region 合并状态
            //int j = 0;
            //bool isContain = false;
            //T_S_S_NoteList = new List<T_S_S_Node>();
            //T_S_S_NoteList.Add(T_S_S_NodeTempList[0]);
            ...
            //    {
            //        if (T_S_S_NodeDelList[i].SuperPointID == T_S_S_NoteList[j].SuperPointID && T_S_S_NodeDelList[i].CumuConnTime == T_S_S_NoteList[j].CumuConnTime && T_S_S_NodeDelList[i].CumuDriveTime == T_S_S_NoteList[j].CumuDriveTime && T_S_S_NodeDelList[i].CumuDayCrewTime == T_S_S_NoteList[j].CumuDayCrewTime && T_S_S_NodeDelList[i].CumuLunch == T_S_S_NoteList[j].CumuLunch && T_S_S_NodeDelList[i].CumuDinner == T_S_S_NoteList[j].CumuDinner && T_S_S_NodeDelList[i].Relax == T_S_S_NoteList[j].Relax && T_S_S_NodeDelList[i].PointType == T_S_S_NoteList[j].PointType && T_S_S_NodeDelList[i].PervSuperID == T_S_S_NoteList[j].PervSuperID && T_S_S_NodeDelList[i].PervCumuConnTime == T_S_S_NoteList[j].PervCumuConnTime && T_S_S_NodeDelList[i].PervCumuDriveTime == T_S_S_NoteList[j].PervCumuDriveTime && T_S_S_NodeDelList[i].PervCumuDayCrewTime == T_S_S_NoteList[j].PervCumuDayCrewTime && T_S_S_NodeDelList[i].PervCumuLunch == T_S_S_NoteList[j].PervCumuLunch && T_S_S_NodeDelList[i].PervCumuDinner == T_S_S_NoteList[j].PervCumuDinner && T_S_S_NodeDelList[i].PervRelax == T_S_S_NoteList[j].PervRelax && T_S_S_NodeDelList[i].PerPointType == T_S_S_NoteList[j].PerPointType)
            //        {
            //            isContain = true;
            //            break;
            //        }
            ...
            //    if (isContain == false)
            //    {
            //        if (T_S_S_NodeDelList[i].PervID == 5562 && T_S_S_NodeDelList[i].ID == 1)
            //        {
            ...
            //////foreach (CrewBase crewbase in CrewBaseList)
            //////{
            //////    for (i = 0; i < T_S_S_NodeTempList.Count; i++)
            //////    {
            //////        if (T_S_S_NodeTempList[i].ID == crewbase.DIDinTSS)
            //////        {
            //////            T_S_S_NodeTempList[i].Isfeasible = 1;
            //////            DPointList.Add(T_S_S_NodeTempList[i]);
            //////        }
            //////    }
            //////}
            //////foreach (T_S_S_Node note in DPointList)
            ...
            //////for (i = 0; i < T_S_S_NodeTempList.Count; i++)
            //////{
            //////    if (T_S_S_NodeTempList[i].Isfeasible == 1)
            //////    {
            //////        T_S_S_NoteList.Add(T_S_S_NodeTempList[i]);
            //////    }
            //////}
            ////#endregion
            //////重新编号1、算出出虚拟起终点外所有的点
            ...
            //List<T_S_S_Node>NewID = new List<T_S_S_Node>();
            //用Hash查找快
            ...
            for (i = 0; i < T_S_S_NoteList.Count; i++)
            {
                ...
                //T_S_S_NoteList = T_S_S_NodeDelList;
                #region 建立时空状态网
                k1 = 1;
            for (i = 0; i < T_S_S_NoteList.Count; i++)
            {
        ...
        public bool AllLineContain()
        {
            ...
            for(i=0;i<LineList.Count;i++)
            {
                for(j=0;j<T_S_S_ArcList.Count;j++)
                {
                    ...
                    {
                        if (T_S_S_ArcList[j].StartTimeCode == 433 && T_S_S_ArcList[j].EndTimeCode == 468)
                        {
                            int aaa = 1;
                        }
